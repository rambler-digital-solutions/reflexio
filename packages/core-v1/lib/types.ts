...
type TriggerPhasePayload2<IR, K extends keyof IR> =
  IR[K] extends BiteStatusWrap<Record<string, unknown>>
...
export type UpdateOnType<ITrigger> = Array<
  | Partial<
      Record<
        keyof ITrigger,
        | TriggerPhaseKeys<ITrigger, keyof ITrigger>
...
> = {
  initOn: ITriggers[BiteName] extends BiteStatusWrap<Record<string, unknown>>
  ...
  watchScope:
    | Array<
        | Partial<
            Record<
              keyof IRootTrigger,
              | TriggerPhaseKeys<IRootTrigger, keyof IRootTrigger>
...
export type MakeBiteType<ITriggers, IState, IRootTrigger> = {
  [key in keyof ITriggers]: {
    ...
    processor: MakeBiteProcessorType<ITriggers, IRootTrigger, key> | null;
  };
};
...
export type WatchArgsType<ITrigger, Tr extends keyof ITrigger> = {
